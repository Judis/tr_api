defmodule I18NAPIWeb.TranslationKeyController do
  ...
  def create(conn, %{"translation_key" => translation_key_params}) do
    with {:ok, %TranslationKey{} = translation_key} <- Translations.create_translation_key(translation_key_params) do
      conn
      |> put_status(:created)
  ...
  def update(conn, %{"id" => id, "translation_key" => translation_key_params}) do
    ...
    with {:ok, %TranslationKey{} = translation_key} <- Translations.update_translation_key(translation_key, translation_key_params) do
      render(conn, "show.json", translation_key: translation_key)
    end
  end
